...
for ii = 1:length(nblades)
    nb = nblades(ii);
    [t(:,ii),px(:,ii),py(:,ii),pt(:,ii),CP(:,ii),CT(:,ii),Wx(ii),Vrel_x(ii),Vrel(ii)] = VAWT(nb,dt,ntime);
end
...
function [t,px_tot,py_tot,pt_tot,CP,CT,Wx,Vrel_x,Vrel] = VAWT(B,dt,ntime)
