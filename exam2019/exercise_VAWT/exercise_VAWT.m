...
blades = [3]; % blades to investigate
...
time_final = 5; % time when to stop the analysis
time_plot_initial = 0.0006; % time when start to plot values
time_plot_final = 4.8; % time when stop to plot the values
t_start = 4.5; % (s) time when start integration
t_stop = 4.8;  % (s) time when stop integration
...
for b=1:blades_size
  B = blades(b); %select the number of blades
  ...
  for n=2:ntime % run over time
    ...
    for i=1:B % run over number of blades
      c = S*R/B;
      theta_i = 2*pi; % theta1(n) + 2*pi*(i - 1)/B;
