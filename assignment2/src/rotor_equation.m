...
delta = 0.5.*asin(2*sqrt(3)/3*Pmech*Ls ./ (flux.^2.*omega_E)); % angle between the back 
% emf and Vg
Ig = sqrt(3).*Pmech./(flux.*omega_E.*cos(delta)*3); % generator current (A)
Vg = flux.*omega_E.*cos(delta)/sqrt(3) - Ig*Rs; % generator tension (V)
